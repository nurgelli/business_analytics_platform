...
@router.get("/rollup_cube/most_benefit_products", response_model=list[MostProfitableProduct])
def get_benefit_products():

    return execute_query("analytics/rollup_cube/most_benefit_products.sql")


@router.get("/materialized/monthly_revenue", response_model=list[MaterializedMonthlyRevenue])
...
@router.get("/materialized/customer_summary", response_model=list[MaterializedCustomerSummary])
