...
def available_years(*frames: pd.DataFrame) -> list[int]:
    ...
    for df in frames:
        ...
        if "full_date" in df.columns:
            years.update(pd.to_datetime(df["full_date"], errors="coerce").dropna().dt.year.astype(int).tolist())
    return sorted(years)
...
def apply_filters(df: pd.DataFrame, year: int | None, region: str, segment: str, category: str) -> pd.DataFrame:
    if df.empty:
        return df
...
with st.sidebar:
    ...
    selected_region = st.selectbox("Region", ["All"] + unique_values(all_frames, "region"))
    selected_segment = st.selectbox("Segment", ["All"] + unique_values(all_frames, "segment"))
    selected_category = st.selectbox("Category", ["All"] + unique_values(all_frames, "category"))
    top_n = st.slider("Top N", min_value=5, max_value=25, value=10, step=5)

    st.divider()
    st.header("Retention")
    retention_years = years or [2021, 2022, 2023]
    base_year = st.selectbox("Base Year", retention_years, index=max(0, len(retention_years) - 2))
    next_year = st.selectbox("Comparison Year", retention_years, index=max(0, len(retention_years) - 1))

filtered_monthly = apply_filters(monthly_df, selected_year, selected_region, selected_segment, selected_category)
filtered_daily = apply_filters(daily_df, selected_year, selected_region, selected_segment, selected_category)
filtered_region = apply_filters(region_df, selected_year, selected_region, selected_segment, selected_category)
filtered_products = apply_filters(products_df, selected_year, selected_region, selected_segment, selected_category)
filtered_category = apply_filters(category_df, selected_year, selected_region, selected_segment, selected_category)
filtered_inventory = apply_filters(inventory_df, selected_year, selected_region, selected_segment, selected_category)
filtered_customers = apply_filters(customers_df, selected_year, selected_region, selected_segment, selected_category)
filtered_lifetime = apply_filters(customer_lifetime_df, selected_year, selected_region, selected_segment, selected_category)
filtered_growth = apply_filters(growth_df, selected_year, selected_region, selected_segment, selected_category)
filtered_moving_avg = apply_filters(moving_avg_df, selected_year, selected_region, selected_segment, selected_category)
filtered_rank = apply_filters(monthly_rank_df, selected_year, selected_region, selected_segment, selected_category)
filtered_running_total = apply_filters(running_total_df, selected_year, selected_region, selected_segment, selected_category)
filtered_rollup = apply_filters(rollup_df, selected_year, selected_region, selected_segment, selected_category)
filtered_cube = apply_filters(cube_df, selected_year, selected_region, selected_segment, selected_category)
filtered_profitable = apply_filters(profitable_products_df, selected_year, selected_region, selected_segment, selected_category)
filtered_mv_monthly = apply_filters(mv_monthly_df, selected_year, selected_region, selected_segment, selected_category)
filtered_mv_customer = apply_filters(mv_customer_df, selected_year, selected_region, selected_segment, selected_category)
...
overview_tab, sales_tab, product_tab, customer_tab, advanced_tab, data_tab = st.tabs(
    ["Overview", "Sales", "Products", "Customers", "Advanced Analytics", "Data Explorer"]
)
...
with overview_tab:
    ...
    c3.metric("Profit Margin", metric_value(kpi.get("profit_margin_pct"), suffix="%", decimals=2))
    ...
    active_customers = filtered_lifetime["customer_id"].nunique() if "customer_id" in filtered_lifetime else 0
    avg_lifetime = filtered_lifetime["lifetime_revenue"].mean() if "lifetime_revenue" in filtered_lifetime else 0
    avg_order_value = filtered_lifetime["average_order_value"].mean() if "average_order_value" in filtered_lifetime else 0
    ...
    with left:
        if not filtered_growth.empty:
            fig = go.Figure()
            fig.add_bar(x=filtered_growth["year"], y=filtered_growth["revenue"], name="Revenue")
            fig.add_scatter(
...
with sales_tab:
    ...
    if not filtered_monthly.empty:
        ...
        monthly_view["period"] = monthly_view["year"].astype(str) + "-" + monthly_view["month"].astype(str).str.zfill(2)
        monthly_view["profit_margin_pct"] = (
            monthly_view["total_profit"] / monthly_view["total_revenue"].replace(0, pd.NA) * 100
        )
        fig = px.line(
            monthly_view,
            x="period",
    ...
    with c2:
        if not filtered_daily.empty:
            fig = px.area(coerce_dates(filtered_daily), x="full_date", y="total_revenue")
            st.plotly_chart(fig, use_container_width=True)
...
with product_tab:
    ...
    st.dataframe(top_records(filtered_inventory, "total_units_sold", top_n), use_container_width=True, hide_index=True)
...
with advanced_tab:
    ...
    if not filtered_moving_avg.empty:
        ...
        fig = go.Figure()
        fig.add_scatter(x=moving_view["full_date"], y=moving_view["revenue"], name="Revenue", mode="lines")
        fig.add_scatter(x=moving_view["full_date"], y=moving_view["moving_avg_7d"], name="7D Moving Avg", mode="lines")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with c1:
        ...
        if not filtered_running_total.empty:
            ...
            fig = px.line(running_view, x="full_date", y="cumulative_revenue", color="category")
            st.plotly_chart(fig, use_container_width=True)
    with c2:
    ...
    with c4:
        ...
        st.dataframe(top_records(filtered_cube, "revenue", top_n), use_container_width=True, hide_index=True)
